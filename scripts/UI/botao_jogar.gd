...
@export_file("res://scenes/fases/fase1/fase1.tscn") var cena_destino: String
...
func _on_pressed():
    if eh_botao_sair:
        get_tree().quit()
    elif cena_destino != "res://scenes/fases/fase1/fase1.tscn":
        get_tree().change_scene_to_file(cena_destino)
    else:
        print("Jogar ", name, " não tem destino configurado!")
